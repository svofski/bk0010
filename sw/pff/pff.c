...
CLUST clst;
{
	...
	case FS_FAT12 :
		...
		ofs = bc % 512; bc /= 512;
		if (ofs != 511) {
			if (disk_readp(buf, (DWORD)fs->fatbase + bc, ofs, 2)) break;
		} else {
			if (disk_readp(buf, (DWORD)fs->fatbase + bc, 511, 1)) break;
			if (disk_readp(buf+1, (DWORD)fs->fatbase + bc + 1, 0, 1)) break;
		}
		wc = LD_WORD(buf);
...
#if _FS_FAT32
	case FS_FAT32 :
		if (disk_readp(buf, (DWORD)fs->fatbase + clst / 128, (WORD)(((WORD)clst % 128) * 4), 4)) break;
...
DWORD* sect;
{
    ...
    *sect = (DWORD)((DWORD)clst) * ((DWORD)fs->csize) + fs->database;
}
...
static
FRESULT create_name (
	dj,		/* Pointer to the directory object */
...
void get_fileinfo (		/* No return code */
	dj,			/* Pointer to the directory object */
	fno	 	/* Pointer to store the file information */
)
DIR *dj;
FILINFO *fno;
{
...
FRESULT follow_path (	/* FR_OK(0): successful, !=0: error code */
	dj,			/* Directory object to return last directory and found object */
...
DWORD sect;
{
	...
	if (!disk_readp(buf, (DWORD)sect, BS_FilSysType, 2) && LD_WORD(buf) == 0x4146)	/* Check FAT12/16 */
		return 0;

#if _FS_FAT32
	if (!disk_readp(buf, (DWORD)sect, BS_FilSysType32, 2) && LD_WORD(buf) == 0x4146)	/* Check FAT32 */
		return 0;
#endif
	return 1;
}
...
FATFS *fs;
{
	...
	bsect = 0;
	fmt = check_fs(buf, (DWORD)bsect);			/* Check sector 0 as an SFD format */
	if (fmt == 1) {						/* Not an FAT boot record, it may be FDISK format */
		/* Check a partition listed in top of the partition table */
		if (disk_readp(buf, (DWORD)bsect, MBR_Table, 16)) {	/* 1st partition entry */
			fmt = 3;
		} else {
...
WORD* br;
{
	...
	*br = 0;
	...
	while (btr)	{									/* Repeat until all data transferred */
		...
		dr = disk_readp(!buff ? 0 : rbuff, (DWORD)fs->dsect, (WORD)(fs->fptr % 512), rcnt);
		if (dr) goto fr_abort;
...
)
{
	...
	*bw = 0;
	...
	if (!btw) {		/* Finalize request */
		if ((fs->flag & FA__WIP) && disk_writep(0, (DWORD)0)) goto fw_abort;
		fs->flag &= ~FA__WIP;
		return FR_OK;
	...
	while (btw)	{									/* Repeat until all data transferred */
		...
		if (disk_writep(p, (DWORD)wcnt)) goto fw_abort;	/* Send data to the sector */
		fs->fptr += wcnt; p += wcnt;				/* Update pointers and counters */
		btw -= wcnt; *bw += wcnt;
		if (((WORD)fs->fptr % 512) == 0) {
			if (disk_writep(0, (DWORD)0)) goto fw_abort;	/* Finalize the currtent secter write operation */
			fs->flag &= ~FA__WIP;
		}
	}
